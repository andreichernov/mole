...
const (
	...
	UserMissing = "no user could be found for server %s"
	KeyMissing  = "key file does not exists: %s: %v"
)
...
func NewServer(user, address, key string) (*Server, error) {
	...
	if user == "" {
		return nil, fmt.Errorf(UserMissing, host)
	}
	...
	if _, err := os.Stat(key); err != nil {
		return nil, fmt.Errorf(KeyMissing, key, err)
	}
