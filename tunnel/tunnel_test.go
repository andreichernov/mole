...
func TestServerOptions(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			"testdata/.ssh/id_rsa",
			nil,
			fmt.Errorf(UserMissing, "172.17.0.1"),
		},
		{
			"user",
			"172.17.0.1",
			"testdata/.ssh/key_does_not_exist",
			nil,
			fmt.Errorf(KeyMissing, "testdata/.ssh/key_does_not_exist", "stat testdata/.ssh/key_does_not_exist: no such file or directory"),
		},
	}
...
func TestTunnelOptions(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"",
			&Server{Name: "local-forward-example"},
			"",
			&Tunnel{
				local:  "127.0.0.1:8443",
				server: &Server{Name: "local-forward-example"},
...
func TestStopTunnel(t *testing.T) {
	...
	tun.client = &ssh.Client{Conn: &MockConn{isConnectionClosed: false}}
	...
	case <-tun.Ready:
		...
		case err := <-result:
			if err != nil {
				t.Errorf("tunnel has stopped with an error: %v", err)
			}

			if c, ok := tun.client.Conn.(*MockConn); ok {
				if c.isConnectionClosed {
					t.Errorf("tunnel has stopped but ssh connection remains open")
				}
			}

		}
	case <-time.After(1 * time.Second):
		t.Error("timeout waiting for tunnel to be stopped")
	}
}
